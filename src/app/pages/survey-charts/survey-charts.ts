...
import { CommonModule } from '@angular/common';
import { NgChartsModule } from 'ng2-charts';
import { SurveyStats } from '../../models/dashboard';
...
export default class SurveyCharts implements OnChanges {
  @Input() surveyStats?: SurveyStats;

  donutData: any = { labels: [], datasets: [] };

  ngOnChanges(changes: SimpleChanges) {
    const s = this.surveyStats;
    const submitted = s?.totalSubmitted ?? 0;
    const pending = s?.totalPending ?? ( (s?.totalEmployees ?? 0) - submitted );
    this.donutData = { labels: ['Submitted', 'Pending'], datasets: [{ data: [submitted, pending] }] };
  }
}
